...
Input::Input(const Input &other)
    : name(other.name), slot(other.slot), width(other.width),
      incomingRequest(other.incomingRequest) {}

std::string Input::toString() const {
    return fmt::format("[ name: {}, slot: {}, width: {}, incomingRequest: {} ]",
                       name, slot, width, incomingRequest);
}
...
void Input::setIncomingRequest(u32 _incomingRequest) {
    this->incomingRequest = _incomingRequest;
}
