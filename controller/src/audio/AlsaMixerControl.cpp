...
bool AlsaMixerControl::applyConfiguredVolume(const std::shared_ptr<creatures::Logger> &log, const AudioConfig &config) {
    ...
    if (result < 0) {
        log->warn("Unable to open ALSA mixer: {}", snd_strerror(result));
        return false;
    }

    result = snd_mixer_attach(mixer.get(), config.alsaMixerCard.c_str());
    if (result < 0) {
        log->warn("Unable to attach ALSA mixer card '{}': {}", config.alsaMixerCard, snd_strerror(result));
        return false;
    }

    result = snd_mixer_selem_register(mixer.get(), nullptr, nullptr);
    if (result < 0) {
        log->warn("Unable to register ALSA mixer elements: {}", snd_strerror(result));
        return false;
    }

    result = snd_mixer_load(mixer.get());
    if (result < 0) {
        log->warn("Unable to load ALSA mixer elements: {}", snd_strerror(result));
        return false;
    }

    snd_mixer_elem_t *element = findPlaybackElement(mixer.get(), config.alsaMixerElement);
    if (element == nullptr) {
        if (config.alsaMixerElement.empty()) {
            log->warn("ALSA mixer card '{}' has no playback-volume element", config.alsaMixerCard);
        } else {
            log->warn("ALSA mixer card '{}' has no playback-volume element named '{}'", config.alsaMixerCard,
                      config.alsaMixerElement);
        }
        return false;
    }
    ...
    if (snd_mixer_selem_get_playback_volume_range(element, &minimum, &maximum) < 0 || maximum <= minimum) {
        log->warn("Unable to read ALSA volume range for '{}'", snd_mixer_selem_get_name(element));
        return false;
    }
    ...
    if (result < 0) {
        log->warn("Unable to set ALSA playback volume on '{}': {}", snd_mixer_selem_get_name(element),
                  snd_strerror(result));
        return false;
    }

    log->info("Set ALSA playback volume to {}% on card '{}', element '{}'", percent, config.alsaMixerCard,
              snd_mixer_selem_get_name(element));
    return true;
#else
    log->warn("Output volume is configured, but this build does not include ALSA mixer support");
    return false;
#endif
}
