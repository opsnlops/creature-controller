...
BaseBuilder::BaseBuilder(std::shared_ptr<Logger> logger, std::string fileName)
    : fileName(std::move(fileName)), logger(logger) {}
...
Result<std::string> BaseBuilder::loadFile(std::shared_ptr<Logger> logger, std::string filename) {
    ...
    if (!fileAccessibleResult.isSuccess()) {
        auto errorMessage = fmt::format("Unable to determine if {} is accessible: {}", filename,
                                        fileAccessibleResult.getError().value().getMessage());
