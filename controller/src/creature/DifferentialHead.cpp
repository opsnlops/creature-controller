...
DifferentialHead::DifferentialHead(std::shared_ptr<creatures::Logger> logger,
...
u16 DifferentialHead::convertToHeadHeight(u16 y) const {
    return convertRange(logger, y, positionMin, positionMax,
                        positionMin + (headOffsetMax / 2),
                        positionMax - (headOffsetMax / 2));
}

int32_t DifferentialHead::convertToHeadTilt(u16 x) const {
    return convertRange(logger, x, positionMin, positionMax,
                        1 - (headOffsetMax / 2), headOffsetMax / 2);
}
...
head_position_t DifferentialHead::calculateHeadPosition(u16 height, int32_t offset) const {
    ...
    logger->trace("calculated head position: height: {}, offset: {} -> {}, {}",
                  height, offset, headPosition.right, headPosition.left);
