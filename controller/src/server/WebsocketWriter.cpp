...
namespace creatures::server {
    ...
    WebsocketWriter::WebsocketWriter(const std::shared_ptr<Logger>& logger,
    ...
    void WebsocketWriter::run() {
        ...
        while(!stop_requested.load()) {
            ...
            // If we're not enabled, just continue and don't send the message. We need
            // to chew things off the queue so that we don't leak memory 😅
            if(!this->enabled) {
